...
<script type="text/javascript">
    ...
    function getLangDate() {
        ...
        document.getElementById("nowTime").innerHTML = "亲爱的${sessionScope.user.userName}，" + timeValue + "好！ 欢迎使用WLY二阶段项目。当前时间为： " + newDate + "　" + week;
    ...
    layui.use(['form', 'element', 'layer', 'jquery'], function () {
        ...
        //json格式: resources/json/newsList.json  (刚开始,后续使用数据库...)
        //todo 最新文章列表(旧)
        ...
        /*$.get("/news/loadAllNews?page=1&limit=10",function(data){
            ...
            for(var i=0;i<length;i++){
                var idValue = data.data[i].id;
                var titleValue=data.data[i].title;
                var createtimeValue=data.data[i].createtime.substring(0,10);
                hotNewsHtml += "<tr ondblclick=viewNews("+idValue+")>"
                    +"<td align='left'><a href='javascript:;'> "+titleValue+"</a></td>"
                    +"<td>"+createtimeValue+"</td>"
                    +"</tr>";
                // alert(idValue+"\t"+titleValue+"\t"+createtimeValue+"\t"+i);
            }
    ...
    function viewNews(id){
        $.get("${whContextPath}/news/loadNewsById",{id:id},function(news){
            layer.open({
                type:1,
                title:'查看公告',
